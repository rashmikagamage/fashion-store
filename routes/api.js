                ...
                bcrypt.hash(req.body.password, salt, (err, hash) => {
                    ...
                    User.create(req.body).then(function (user) {
                        ...
                        res.status(200).send(JSON.stringify({
                            success: "A verification email has been sent to your email account",
                            code: 'reg',
                            user: user
                        }));
...
router.post("/addItemToWishList/:id", async (req, res) => {   // add a items for wishlist
    ...
    try {
        ...
        const itemAdd = {
            itemID: item._id, itemName: item.name, mainCategory: item.mainCategory, price: item.price,
        ...
        if (exists) {
            ...
            res.send(JSON.stringify({
                message: "add item successfully to wishlist",
...
router.post("/items", upload.array('productImage', 4), (req, res) => {   // add an item
    ...
    Product.create(product)
        .then(function (products) {
            ...
            res.status(200).send(JSON.stringify({
                success: "Item has been addedd successfully",
                code: 'item',
                user: products
            }));
            ...
            User.create(managerData).then(function (manager) {
                ...
                token.save(function (err) {
                    ...
                    var transporter = nodemailer.createTransport({
                        ...
                        port: 25,
                        secure: false,
                        auth: {user: 'hasithakeshana9900@gmail.com', pass: '9812sliit'},
                        tls: {rejectUnauthorized: false}
                    });
                    var mailOptions = {
                        from: 'hasithakeshana9900@gmail.com',
                        to: manager.email,
                    ...
                    transporter.sendMail(mailOptions, function (err) {
                        ...
                        res.status(200).send({
                            message: 'A verification email has been sent to ' + manager.email + '.',
                            code: "success"
                        });
                    });
                });
                    ...
                    User.create(AdminData).then(function (user) {
                        ...
                        token.save(function (err) {
                            ...
                            var transporter = nodemailer.createTransport({
                                ...
                                port: 25,
                                secure: false,
                                auth: {user: process.env.EMAIL, pass: process.env.EMAIL},
                                tls: {rejectUnauthorized: false}
                            });
                            var mailOptions = {
                                from: process.env.EMAIL,
